...
public class ItemTable {
	...
	public Object createitem(String string) {
		...
		if (result) {
			...
			logger.info(String.format("Operation:Create New Item;Item Info:[%s,%s];State:Success", string,
					String.valueOf(flag + 1)));
			return newitem;
			
		} else {
			result = false;
			logger.info(
					String.format("Operation:Create New Item;Item Info:[%s,%s];State:Fail;Reason:No such ISBN existed.",string, "N/A"));
	...
	public Object delete(String string, String string2) {
		// Since the itemid and copynumber in is automatically assigned to the
		// item,upon its creation.
		// Each item has a unique itemid and copynumber.Even it is deleted,they
		// can not be assigned to other item.
		// To maintain the correctness of the data,here instead delete index
		// from the List.
		...
		if (flag != 0) {
			...
			if (loan) {
				...
				logger.info(String.format(
						"Operation:Delete Item;Item Info:[%s,%s];State:Fail;Reason:The item is currently on loan.",
						string, string2));
			}
		} else {
			result = "The Item Does Not Exist";
			logger.info(
					String.format("Operation:Delete Item;Item Info:[%s,%s];State:Fail;Reason:The Item Does Not Exist.",
							string, string2));
		}
		return result;
	}
	...
	public void deleteAll(String string) {
		for (int i = 0; i < itemList.size(); i++) {
			if (string.equalsIgnoreCase(itemList.get(i).getISBN())) {
				...
				logger.info(String.format("Operation:Delete Item Due to Title Deletion;ISBN Info:[%s];State:Success",
						string));
			}
		}
